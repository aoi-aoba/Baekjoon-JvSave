...
void bind() {
    ...
    if (plus_v.size() > 0) {
        for (int i = 0; i + 1 < plus_v.size(); i += 2)
            v.push_back(plus_v[i] * plus_v[i + 1]);
        if (plus_v.size() % 2 == 1)
            v.push_back(plus_v[plus_v.size() - 1]);
    }
    // 1은 곱하면 손해이므로 따로 합으로 처리
    if (ones > 0) v.push_back(ones);
}
...
int main(void) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        else if (temp > 0) plus_v.push_back(temp);
        else zeros++;
    }
